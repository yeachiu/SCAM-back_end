...
public interface SysUserMapper extends BaseMapper<SysUser> {
    ...
    @Select("SELECT u.id, u.username FROM sys_user u WHERE u.id NOT IN "+
            "(SELECT uid FROM user_auth)")
    List<SimpleUserVO> unauthList();

    @Select("SELECT u.id, u.username, auth.real_name AS realName, u.avatar FROM sys_user u, user_auth auth WHERE u.id = auth.uid AND u.id = #{id}")
    SimpleUserVO findSimpleVOById(String id);

    @Select("SELECT u.id, u.username, auth.real_name AS realName, u.avatar FROM sys_user u, user_auth auth WHERE u.id = auth.uid AND u.id IN (SELECT user_id FROM activity_admins WHERE activity_id = #{actiId})")
